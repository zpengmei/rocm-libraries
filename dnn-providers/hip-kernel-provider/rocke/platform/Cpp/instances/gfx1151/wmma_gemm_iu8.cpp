...
#define ROCKE_WMMA_IU8_DEFAULT_NAME "rocke_wmma_gemm_iu8"
...
#define ROCKE_WMMA_IU8_K_PER_I32 4 /* _K_PER_I32: int8 K-values packed per i32 */
...
static void rocke_wmma_gemm_iu8_set_reason(char* reason, size_t reason_cap, const char* msg)
{
                                       ...
                                       size_t reason_cap)
{
    ...
    if(spec == NULL)
    {
        rocke_wmma_gemm_iu8_set_reason(reason, reason_cap, "null spec");
        return false;
    }
    if(arch == NULL)
    {
    ...
    if(target == NULL)
    {
        snprintf(buf,
                 ...
                 arch ? arch : "None",
                 arch ? "'" : "");
        rocke_wmma_gemm_iu8_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    rocke_wmma_gemm_iu8_set_reason(reason, reason_cap, "ok");
    return true;
}
                                              ...
                                              const char* arch)
{
    return ckc::guard_builder(b, [&]() -> rocke_kernel_def_t* {
        ...
        if(!rocke_wmma_gemm_iu8_is_valid_spec(spec, arch, reason, sizeof(reason)))
        {
            char msg[ROCKE_ERR_MSG_CAP];
            ROCKE_ERR_SNPRINTF(msg, sizeof(msg), "invalid iu8 WMMA GEMM spec: %s", reason);
        ...
        /* ---- kernel params -- */
        {
            ...
            opts.align = 16;
            opts.align_set = true;
            C = rocke_b_param(b, "C", ptr_i32, &opts);
        ...
        rocke_b_region_enter(b, loop.body);
        {
            ...
            nacc = rocke_b_mma(b, ROCKE_WMMA_IU8_OP_ID, a_frag, b_frag, acc_v, NULL, 0);
            /* b.scf_yield(nacc) */
            yield_vals[0] = nacc;
        ...
        for(i = 0; i < 8; ++i)
        {
            ...
            rocke_b_global_store(b, C, idx, elem, /*align=*/-1);
        }
...
/* ===================================================================== *
 *  rocke_build_wmma_gemm_iu8_new -- init builder with spec.kernel_name()
 *  then build.
 * ===================================================================== */
                                                  ...
                                                  const char* arch)
{
    return ckc::guard_builder(b, [&]() -> rocke_kernel_def_t* {
        ...
        if(rocke_wmma_gemm_iu8_kernel_name(spec, name, sizeof(name)) != ROCKE_OK)
        {
            return NULL;
        }
        ...
        return rocke_build_wmma_gemm_iu8(b, spec, arch);
    });
}
...
/* ===================================================================== *
 *  rocke_wmma_gemm_iu8_lower_to_llvm -- build + lower to .ll convenience.
                                                 ...
                                                 size_t err_cap)
{
    ...
    kernel = rocke_build_wmma_gemm_iu8_new(&b, spec, arch);
    if(kernel == NULL)
    {
