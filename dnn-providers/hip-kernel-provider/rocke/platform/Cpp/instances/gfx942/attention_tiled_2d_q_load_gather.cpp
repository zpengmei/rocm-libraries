...
void rocke_gfx942_attn2d_emit_q_load(rocke_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    for(li = 0; li < Q_VECS_PER_THREAD; ++li)
    {
         ...
         * The VALID_ROWS guard is only needed when NQK does not divide BLOCK_M (i.e.
         * VALID_ROWS < BLOCK_M); otherwise every row index is already in-bounds and the
         * guard is always true — omitting it avoids a redundant cmp+and on the hot path. */
        {
