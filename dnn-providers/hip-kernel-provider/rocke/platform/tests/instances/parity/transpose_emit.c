 ...
 * rocke_transpose2d_spec_t identically to the Python emitter transpose_emit.py,
 * builds into a fresh IRBuilder via rocke_build_transpose2d_new (the C build
...
static int make_spec(int idx, rocke_transpose2d_spec_t* spec)
{
    *spec = rocke_transpose2d_spec_default();
...
int main(int argc, char** argv)
{
    ...
    rocke_kernel_def_t* kernel = rocke_build_transpose2d_new(&b, &spec, arch);
    if(!kernel)
    {
