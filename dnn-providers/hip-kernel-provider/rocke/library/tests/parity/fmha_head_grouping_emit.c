 ...
 * argv[1] (the config index 0..5), builds rocke_fmha_head_grouping_spec_t
 * identically to the Python emitter fmha_head_grouping_emit.py, builds the
 * kernel via rocke_build_fmha_fwd_head_grouping (arch gfx950) and lowers via
 * rocke_lower_kernel_to_llvm (flavor AUTO) and prints the .ll to stdout so the
...
static int make_spec(int idx, rocke_fmha_head_grouping_spec_t* spec)
{
    ...
    case 0:
        ...
        *spec = rocke_fmha_head_grouping_spec_make(common, 16, 16);
        break;
    case 1:
        shape = rocke_fmha_shape_default(128, 16, 1);
        common = rocke_fmha_common_spec_default(shape);
        common.dtype = "f16";
        common.mask_mode = ROCKE_FMHA_MASK_CAUSAL;
        *spec = rocke_fmha_head_grouping_spec_make(common, 32, 32);
        break;
    case 2:
        shape = rocke_fmha_shape_default(64, 8, 4);
        common = rocke_fmha_common_spec_default(shape);
        common.dtype = "bf16";
        common.mask_mode = ROCKE_FMHA_MASK_NONE;
        *spec = rocke_fmha_head_grouping_spec_make(common, 32, 32);
    ...
    case 3:
        ...
        *spec = rocke_fmha_head_grouping_spec_make(common, 16, 16);
        break;
    case 4:
        shape = rocke_fmha_shape_default(128, 8, 2);
        common = rocke_fmha_common_spec_default(shape);
        common.dtype = "bf16";
        common.mask_mode = ROCKE_FMHA_MASK_NONE;
        *spec = rocke_fmha_head_grouping_spec_make(common, 48, 64);
        break;
    case 5:
        shape = rocke_fmha_shape_default(64, 24, 8);
        common = rocke_fmha_common_spec_default(shape);
        common.dtype = "f16";
        common.mask_mode = ROCKE_FMHA_MASK_CAUSAL;
        *spec = rocke_fmha_head_grouping_spec_make(common, 64, 64);
        break;
    default:
        return -1;
    }
...
int main(int argc, char** argv)
{
    ...
    rocke_kernel_def_t* kernel = rocke_build_fmha_fwd_head_grouping(&kb, &spec, "gfx950");
    if(kernel == NULL)
    {
