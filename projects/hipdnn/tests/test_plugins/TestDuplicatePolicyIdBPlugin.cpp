...
/**
 * @file TestDuplicatePolicyIdBPlugin.cpp
 * @brief Test plugin with duplicate policy ID (plugin B)
 *
 * This plugin returns the same policy name as TestDuplicatePolicyIdAPlugin,
...
class DuplicatePolicyIdBPlugin : public TestHeuristicPluginBase
{
public:
    const char* getPolicyName() const override
    {
        // Same name as plugin A -> same policy ID
        return "TestDuplicatePolicyName";
    }

    const char* getPluginVersion() const override
    {
        return "1.0.0-duplicate-b";
    }
};
...
{
    TestHeuristicPluginBase::setInstance(std::make_unique<DuplicatePolicyIdBPlugin>());
}
