...
// GPU-only end-to-end round-trip test for the autotune config file (sdpa_bwd).
//
// One real SDPA-backward graph is autotuned (restricted to a single tunable
// engine A so the winner is deterministic) and the winner is written to a JSON
// config. A fresh graph then loads that config via HIPDNN_HEUR_CONFIG_PATH and
// build() re-selects the engine. The discriminator is the engine-ID FLIP: with
// the file as-written the backend selects A; after rewriting the file's
// engine_name to a different candidate B, a fresh build() selects B. Selection
// tracking the file content across two engines (the only thing that changed)
// proves the config drove the choice.
//
...
//
// The entire translation unit is gated on HIPDNN_ENABLE_SDPA: OperationType::
// SDPA_BACKWARD and the SDPA-backward factory builder only exist when SDPA is
// enabled, so with SDPA OFF this file compiles to an empty namespace.
...
class IntegrationAutotuneConfigRoundTripSdpaBwd : public hipdnn_tests::AutotuneIntegrationFixture
{
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the sdpa_bwd
        // node the Config policy reorders the candidate engines (preferred first)
        // and the outer policy loop stops there; SelectionHeuristic::StaticOrdering
        // is the canonical fallback that enumerates the engine-plugin candidates
        // when Config declines. The candidate set (engines A and B) comes from the
    ...
    std::filesystem::path _configFile
        = std::filesystem::temp_directory_path() / "test_autotune_config_round_trip_sdpa_bwd.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripSdpaBwd, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::SDPA_BACKWARD;
