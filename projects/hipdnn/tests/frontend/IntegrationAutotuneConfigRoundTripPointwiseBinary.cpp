...
// binary, mode ADD).
//
// One real pointwise-binary graph is autotuned (restricted to a single tunable
...
//
// Pointwise binary exercises the arity-derived "pointwise_binary" op string and
// the two-input positional key (in_0, in_1).
//
...
class IntegrationAutotuneConfigRoundTripPointwiseBinary
...
protected:
    ...
    std::filesystem::path _configFile = std::filesystem::temp_directory_path()
                                        / "test_autotune_config_round_trip_pointwise_binary.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripPointwiseBinary, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::POINTWISE_BINARY;
