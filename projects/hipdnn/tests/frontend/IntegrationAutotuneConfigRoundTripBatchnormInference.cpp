...
// (batchnorm_inference).
//
// One real batchnorm-inference graph is autotuned (restricted to a single
// tunable engine A so the winner is deterministic) and the winner is written to
// a JSON config. A fresh graph then loads that config via HIPDNN_HEUR_CONFIG_PATH
// and build() re-selects the engine. The discriminator is the engine-ID FLIP:
// with the file as-written the backend selects A; after rewriting the file's
...
class IntegrationAutotuneConfigRoundTripBatchnormInference
...
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the batchnorm
    ...
    std::filesystem::path _configFile
        = std::filesystem::temp_directory_path()
          / "test_autotune_config_round_trip_batchnorm_inference.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripBatchnormInference, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::BATCHNORM_INFERENCE;
