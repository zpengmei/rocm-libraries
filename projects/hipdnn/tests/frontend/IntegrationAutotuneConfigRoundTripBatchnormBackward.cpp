...
// (batchnorm_backward).
//
// One real batchnorm-backward graph is autotuned (restricted to a single tunable
...
class IntegrationAutotuneConfigRoundTripBatchnormBackward
...
protected:
    ...
    std::filesystem::path _configFile = std::filesystem::temp_directory_path()
                                        / "test_autotune_config_round_trip_batchnorm_backward.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripBatchnormBackward, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::BATCHNORM_BACKWARD;
