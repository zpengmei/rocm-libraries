...
// GPU-only end-to-end round-trip test for the autotune config file (pointwise
// unary, mode RELU_FWD).
//
// One real pointwise-unary graph is autotuned (restricted to a single tunable
...
//
// Pointwise unary is the first single-required-input op to round-trip; it
// exercises the relaxed backend match guard (1-input floor) plus the
// arity-derived "pointwise_unary" op string.
//
...
class IntegrationAutotuneConfigRoundTripPointwiseUnary
...
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the pointwise
    ...
    std::filesystem::path _configFile = std::filesystem::temp_directory_path()
                                        / "test_autotune_config_round_trip_pointwise_unary.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripPointwiseUnary, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::POINTWISE_UNARY;
