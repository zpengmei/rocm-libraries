...
// GPU-only end-to-end round-trip test for the autotune config file (layernorm).
//
// One real layernorm graph is autotuned (restricted to a single tunable engine A
// so the winner is deterministic) and the winner is written to a JSON config. A
...
class IntegrationAutotuneConfigRoundTripLayernorm : public hipdnn_tests::AutotuneIntegrationFixture
{
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the layernorm
    ...
    std::filesystem::path _configFile
        = std::filesystem::temp_directory_path() / "test_autotune_config_round_trip_layernorm.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripLayernorm, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::LAYERNORM;
