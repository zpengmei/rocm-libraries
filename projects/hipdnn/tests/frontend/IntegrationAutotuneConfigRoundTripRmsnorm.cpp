...
// GPU-only end-to-end round-trip test for the autotune config file (rmsnorm).
//
// One real rmsnorm graph is autotuned (restricted to a single tunable engine A
...
class IntegrationAutotuneConfigRoundTripRmsnorm : public hipdnn_tests::AutotuneIntegrationFixture
{
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the rmsnorm
    ...
    std::filesystem::path _configFile
        = std::filesystem::temp_directory_path() / "test_autotune_config_round_trip_rmsnorm.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripRmsnorm, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::RMSNORM;
