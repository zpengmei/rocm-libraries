...
// GPU-only end-to-end round-trip test for the autotune config file (matmul).
//
// One real matmul graph is autotuned (restricted to a single tunable engine A so
...
class IntegrationAutotuneConfigRoundTripMatmul : public hipdnn_tests::AutotuneIntegrationFixture
{
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the matmul
    ...
    std::filesystem::path _configFile
        = std::filesystem::temp_directory_path() / "test_autotune_config_round_trip_matmul.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripMatmul, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::MATMUL;
