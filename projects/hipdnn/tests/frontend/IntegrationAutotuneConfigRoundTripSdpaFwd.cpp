...
// GPU-only end-to-end round-trip test for the autotune config file (sdpa_fwd).
//
// One real SDPA-forward graph is autotuned (restricted to a single tunable
...
// SDPA_FORWARD and the SDPA-forward factory builder only exist when SDPA is
...
class IntegrationAutotuneConfigRoundTripSdpaFwd : public hipdnn_tests::AutotuneIntegrationFixture
{
protected:
    void SetUp() override
    {
        ...
        // SelectionHeuristic::Config FIRST. When the config matches the sdpa_fwd
    ...
    std::filesystem::path _configFile
        = std::filesystem::temp_directory_path() / "test_autotune_config_round_trip_sdpa_fwd.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripSdpaFwd, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::SDPA_FORWARD;
