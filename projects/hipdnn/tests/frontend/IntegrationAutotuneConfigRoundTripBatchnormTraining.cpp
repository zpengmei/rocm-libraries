...
// (batchnorm_training).
//
// One real batchnorm-training graph is autotuned (restricted to a single tunable
...
class IntegrationAutotuneConfigRoundTripBatchnormTraining
...
protected:
    ...
    std::filesystem::path _configFile = std::filesystem::temp_directory_path()
                                        / "test_autotune_config_round_trip_batchnorm_training.json";
...
TEST_F(IntegrationAutotuneConfigRoundTripBatchnormTraining, EngineIdFlipsWithConfigContent)
{
    ...
    const auto op = hipdnn_test_sdk::utilities::OperationType::BATCHNORM_TRAINING;
