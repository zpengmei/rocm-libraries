...
// GPU-free compile-time guard against enum-numbering drift. These values are a
// fixed ABI commitment matching the upstream extension namespace; if any value
// drifts, benchmarking would silently bind to the wrong descriptor/attribute.
// This static_assert compiles on ANY host (no GPU/runtime required).
static_assert(
    static_cast<int>(HIPDNN_BACKEND_PROFILING_CONTROL_EXT) == 34,
    "HIPDNN_BACKEND_PROFILING_CONTROL_EXT must equal 34 (ABI/enum-numbering commitment).");
static_assert(static_cast<int>(HIPDNN_ATTR_PROFILING_HANDLE_EXT) == 60400,
              "HIPDNN_ATTR_PROFILING_HANDLE_EXT must equal 60400.");
static_assert(static_cast<int>(HIPDNN_ATTR_PROFILING_START_EXT) == 60401,
              "HIPDNN_ATTR_PROFILING_START_EXT must equal 60401.");
static_assert(static_cast<int>(HIPDNN_ATTR_PROFILING_STOP_EXT) == 60402,
              "HIPDNN_ATTR_PROFILING_STOP_EXT must equal 60402.");
static_assert(static_cast<int>(HIPDNN_ATTR_PROFILING_ELAPSED_MS_EXT) == 60403,
              "HIPDNN_ATTR_PROFILING_ELAPSED_MS_EXT must equal 60403.");
static_assert(static_cast<int>(HIPDNN_ATTR_PROFILING_DEVICE_SYNC_EXT) == 60404,
              "HIPDNN_ATTR_PROFILING_DEVICE_SYNC_EXT must equal 60404.");
