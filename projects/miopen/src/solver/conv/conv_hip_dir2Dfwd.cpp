...
MIOPEN_DECLARE_ENV_VAR_BOOL(MIOPEN_DEBUG_CONV_DIRECT_HIP_FWD)
...
/// This prevents errors in ConvHipDirectFwd::GetSolution(),
/// or during kernel build. Reproduces logic from GetSolution()
/// and some logic from the corresponding kernel source.
/// The cases which lead to errors can be later omitted from the search.
/// \todo Get rid the duplication of code where possible.
bool ConvHipDirectFwd::IsValidPerformanceConfig(const ExecutionContext&,
                                                ...
                                                const LegacyPerformanceConfig& config) const
{
    ...
    // Reproduces some build logic (preprocessing computations) from the kernel source.
    // Variables whose names begin with "mlo_" represent corresponding "MLO_" macros,
    // e.g. mlo_in_lcl_width here represents MLO_IN_LCL_WIDTH macro in the kernel source.
...
ConvSolution ConvHipDirectFwd::BaseGetSolution(const ExecutionContext& ctx,
                                               const ProblemDescription& problem,
                                               const LegacyPerformanceConfig& config)
{
    ...
    kernel_params.kernel_file = "MIOpenConvDirUniHip.cpp";
...
ConvSolution ConvHipDirectFwd::GetSolution(const ExecutionContext& ctx,
                                           const ProblemDescription& problem,
                                           const LegacyPerformanceConfig& config) const
{
