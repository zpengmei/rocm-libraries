...
using DeviceOpInstance = ck::tensor_operation::device::DeviceGemmMultiD_Xdl_CShuffle_V3<
    ...
    FP8, FP8, DsDataType, BF16, F32, F32,
...
int main(int argc, char* argv[])
{
    ...
    ck::index_t StrideD0 = StrideD;
    ck::index_t StrideD1 = StrideD;
    ...
    auto quantize_a = [&](auto& q_tensor, auto& scale_tensor, ck::index_t rows, ck::index_t cols) {
        std::mt19937 gen(11939);
        std::normal_distribution<float> dist(0.0f, 1.0f);

        for(ck::index_t row = 0; row < rows; ++row)
        {
            float amax = 0.0f;
            std::vector<float> values(cols);
            for(ck::index_t col = 0; col < cols; ++col)
            {
                const float value = ck::type_convert<float>(ck::type_convert<BF16>(dist(gen)));
                values[col]       = value;
                amax              = std::max(amax, std::abs(value));
            }

            const float scale    = amax == 0.0f ? 1.0f : amax / fp8_max;
            scale_tensor(row, 0) = scale;

            for(ck::index_t col = 0; col < cols; ++col)
            {
                q_tensor(row, col) = ck::type_convert<FP8>(values[col] / scale);
            }
        }
    };

    auto quantize_b = [&](auto& q_tensor, auto& scale_tensor, ck::index_t rows, ck::index_t cols) {
        std::mt19937 gen(27183);
        std::normal_distribution<float> dist(0.0f, 1.0f);
        ...
        for(ck::index_t row = 0; row < rows; ++row)
        {
            ...
            const float scale    = amax == 0.0f ? 1.0f : amax / fp8_max;
            scale_tensor(0, row) = scale;

            for(ck::index_t col = 0; col < cols; ++col)
            {
                q_tensor(col, row) = ck::type_convert<FP8>(values[col] / scale);
            }
        }
    };
    ...
    case 0: break;
    case 1:
        // Match Aiter's per-token quant path: x_scale is per M row and w_scale is per N row.
        quantize_a(a0_m_k, d1_m_n, M, K);
        quantize_b(b0_k_n, d0_m_n, N, K);
        break;
    default: quantize_a(a0_m_k, d1_m_n, M, K); quantize_b(b0_k_n, d0_m_n, N, K);
    }
    ...
    if(do_verification)
    {
        ...
        return ck::utils::check_err(
                   e_m_n_device_result, e_m_n_host_result, "Error: Incorrect results!", 1e-2, 1e-2)
                   ? 0
                   : 1;
    }
